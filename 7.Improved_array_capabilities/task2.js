...
function collect(arr) {
    if (arguments.length !== 1) throw new Error('Must be 1 argument');
    if (!Array.isArray(arr)) throw new Error('Must be an array');
    let result = arr.flat(Infinity);
    if (result.some(item => typeof item !== 'number')) throw new Error('Some element not a number');
    return result.length === 0 ? 0 : result.reduce((acc, item) => acc + item);
}
// Решение
