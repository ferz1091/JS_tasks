...
function compose(...funcs) {
    return (arg) => {
        ...
        for (let func of funcs) {
            if (typeof func !== 'function') throw new Error('All arguments must be functions');
        }
        for (let counter = funcs.length - 1; counter >= 0; counter--) {
            if (funcs[counter]() === undefined) throw new Error('Function did\'t return a value');
            if (counter === funcs.length - 1) {
                arr[counter] = funcs[counter](arg);
            }
            else {
